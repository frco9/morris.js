class Morris.Grid extends Morris.EventEmitter
  ...
  # Handle treatment over new data to update the data series and redraw the chart.
  #
  updateData: (data, startDate = 0, endDate = 0, redraw = true) ->
    ...
    if @options.parseTime
      ...
      @options.sortedData = true
      
    if @options.sortedData
      if !startDate? or startDate == 0
        startDate = data[0][@options.xkey]
      else
        startDate = startDate.getTime()
      if !endDate? or endDate == 0
        endDate = data[data.length-1][@options.xkey]
      else
        endDate = endDate.getTime()

      @data = []
      for row, index in data
        if row[@options.xkey] >= data[0][@options.xkey]
          [ret, ymin, ymax] = @parseData(row, index, ymin, ymax)
          @data.push ret
    else
    ...
    # if startDate? and startDate != 0 and endDate? and endDate != 0
    #   minIdx = 0
    #   maxIdx = 0
    #   $.each
    #     @data
    ...
    #   @data = @data.splice(minIdx, maxIdx-minIdx)
  ...
  # Add a ykey to the data series and redraw the chart.
  #
  addYkey: (ykey, label, startDate = 0, endDate = 0, redraw = true) ->
    added = false
    ...
    $.map @options.data
      , (elementOfArray, indexInArray) =>
        if elementOfArray.hasOwnProperty(ykey)
          added = true
    if added == true
      @options.ykeys.push(ykey)
      @options.labels.push(label)
      @updateData(@options.data, startDate, endDate)
    
    

  # Remove a ykey to the data series and redraw the chart.
  #
  removeYkey: (ykey, label, startDate = 0, endDate = 0, redraw = true) ->
    if $.inArray(ykey, @options.ykeys) == -1
      return
    
    @options.ykeys.splice($.inArray(ykey, @options.ykeys),1);
    @options.labels.splice($.inArray(label, @options.labels),1);
  ...
  redraw: ->
    ...
    @drawGrid()
    @drawGoals()
    @drawMin() if @options.minVal
    @drawMax() if @options.maxVal
    @drawEvents()
    @draw() if @draw
  ...
  drawMin: ->
    color = @options.minLineColor
    yvals = []
    @data.map (el)->
      return el.y.map (il)->
        yvals.push il if !isNaN(il);
    if yvals.length != 0
      ymin = Math.min.apply(Math, yvals);
      @drawHLine(ymin, color)

  drawMax: ->
    color = @options.maxLineColor
    yvals = []
    @data.map (el)->
      return el.y.map (il)->
        yvals.push il if !isNaN(il);
    if yvals.length != 0
      ...
      @drawHLine(ymax, color)
      
  drawHLine: (y, color)->
    @drawGoal(y, color)
    text = @raphael.text(@left-@options.padding/2, @transY(y), y)
      .attr('font-size', @options.gridTextSize)
      .attr('font-family', @options.gridTextFamily)
      .attr('font-weight', @options.gridTextWeight)
      .attr('fill', color)
    ...
    box = text.getBBox();
    @raphael.rect(box.x, box.y, box.width, box.height/1.5)
      .attr('fill', @options.gridBgColor)
      .attr('stroke-width', 0)
    text.toFront();
  ...
  drawYAxisLabel: (xPos, yPos, text, color = @options.gridTextColor) ->
