class Morris.Line extends Morris.Grid
  ...
  hitTest: (x, y) ->
    ...
    for m, indexy in @data[index]._y
      break if Math.abs(y-m) < 0.001
    [index, indexy]
  ...
  #
  # @private
  displayHoverForRow: (index, indexy = -1) ->
    if index?
      @hover.update(@hoverContentForRow(index, indexy)...)
      @hilight(index, indexy)
    else
      @hover.hide()
      @hilight()
  ...
  #
  # @private
  hoverContentForRow: (index, indexy = -1) ->
    row = @data[index]
  ...
  _drawPointFor: (index) ->
    ...
    for row in @data
      ...
      if row._y[index]?
        ...
        circle.mouseover(@onHoverMove)
        circle.mouseout(@onHoverOut)
  ...
  hilight: (index, indexy = -1) =>
    if @prevHilight isnt null and @prevHilight isnt index
      if indexy == -1
        ...
        if @seriesPoints[indexy][@prevHilight]
          @seriesPoints[indexy][@prevHilight].animate @pointShrinkSeries(indexy)
    if index isnt null and @prevHilight isnt index
      if indexy == -1
        ...
        if @seriesPoints[indexy][index]
          @seriesPoints[indexy][index].animate @pointGrowSeries(indexy)
    @prevHilight = index
